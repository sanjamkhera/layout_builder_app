...
lib/
├── bloc/
│   ├── layout_bloc.dart      # BLoC that manages layout state and handles all events
│   ├── layout_events.dart     # All user actions (Add, Move, Resize, Delete, Tab operations)
│   └── layout_state.dart      # Current app state (layouts, activeTab, isLoading, error)
├── models/
│   ├── widget_model.dart      # Single widget data (id, type, position, size)
│   └── layout_model.dart       # Complete layout data (tabId, tabName, widgets list)
├── repository/
│   └── layout_repository.dart # Firestore operations (save/load/delete layouts)
├── presentation/
│   ├── layout_builder_screen.dart  # Main screen with AppBar, tabs, palette, and canvas
│   ├── tabs_control.dart           # Tab management UI (create, switch, delete, rename)
│   ├── canvas_widget.dart          # Main canvas area with zoom/pan and drag-drop handling
│   ├── draggable_canvas_widget.dart # Individual widget on canvas (moveable and resizable)
│   ├── widget_palette.dart         # Sidebar/top bar with draggable widgets (A, B, C, D)
│   ├── resize_handle.dart          # Resize handles for widgets (8 handles: corners + edges)
│   ├── dot_grid_painter.dart       # Grid pattern background painter for canvas
│   ├── zoom_controls.dart          # Zoom in/out/reset controls overlay
│   ├── widget_colors.dart          # Color mapping for different widget types
│   └── test_screen.dart            # Test screen for Firestore sync verification
├── firebase_options.dart      # Firebase configuration (generated by FlutterFire CLI)
└── main.dart                  # App entry point (Firebase init, Anonymous Auth, MyApp)
```
